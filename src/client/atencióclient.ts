...
function getFormData(): SolicitudData | null {
  ...
  const textarea = document.querySelector<HTMLTextAreaElement>('#sc-form textarea');
  ...
  if (!nom || !cognoms || !usuari || !correu || !categoria || categoria === 'Categoria del problema' || !descripcio) {
    return null;
  }
...
function reset(): void {
  ...
  form.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>('input, textarea').forEach(e => e.value = '');
...
function pill(el: HTMLElement): void {
  document.querySelectorAll('.sc-pill').forEach(p => p.classList.remove('act'));
...
function faq(btn: HTMLElement): void {
  ...
  document.querySelectorAll('.sc-fi.open').forEach(i => i.classList.remove('open'));
