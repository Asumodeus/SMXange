...
export const getCursos = async () => {
  // El "async" defineix que la funció és asíncrona.
  try {
    //Si tot va bé fa aquesta funció
...
export const getCursoById = async (id: number) => {
  try {
    ...
    if (rows.length === 0) {
      // El lenght ens diu quants elemsents hi ha a la taula, si és 0 vol dir que no existeix cap curs amb aquest ID
      return Response.json(
